	...
	public function action_update($action = null)
	{
        ...
        if($action == 'add-con')
        {
            ...
            if($check)
            {
                ...
                $insert = $this->curd_model->insert('payment', $update_data);
                if($insert)
                {
                    $error['success'] = true;
                }
		...
		else if($action == "update-con")
		{
            ...
            if($check)
            {
                ...
                $insert = $this->curd_model->update_table('payment', $update_data, array('id'=>$frmdata['edt_id']));
				if($insert)
                {
                    $error['success'] = true;
                }
        ...
        else if($action == "change-status")
        {
            ...
            if($check)
            {
                ...
                $insert = $this->curd_model->update_table('payment', $upload_data, array('id'=>$frmdata['id']));
				if($insert)
                {
                    $error['success'] = true;
                }
