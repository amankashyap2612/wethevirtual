...
class News extends BaseController
{
	...
	public function action_update($action = null)
	{
		...
		{
			$check = $this->validate([
                'news_id' => ['rules' =>  'required','errors' =>  ['required' => 'id is required']],
                'headline' => ['rules' =>  'required','errors' =>  ['required' => 'headline is required']],
                'link' => ['rules' =>  'required','errors' =>  ['required' => 'link is required']],
            ]);
            if($check)
            {
                $frmdata = mysql_clean($frmdata);
                ...
                $insert = $this->curd_model->update_table('news', $update_data, array('id'=>$frmdata['news_id']));
				if($insert)
                {
                    $error['success'] = true;
                }
        ...
        else if($action == "change-status-news")
        {
            $check = $this->validate([
