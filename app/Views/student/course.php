	...
	?>
		...
		<section>
			<div class="container">
				<div class="row">
					<div class="col-md-12">
						<h3>Login Required again for verification. please use same login to start study.</h3>
					</div>
					<div class="col-md-12">
						<h3>Your Active Course</h3>
						<table class="table table-striped table-bordered tbl-shopping-cart">
							...
							<tbody>
							<?php if(count($active_course)){ foreach($active_course as $crs){ ?>
									<tr>
											<td class="product-thumbnail"><img alt="<?=$crs->course_code;?>" title="<?=$crs->title;?>" src="<?=base_url("images/".$image_detail[$crs->img_id]);?>"></td>
											<td><?= strtoupper($crs->title); ?></td>
											<td>
												<form class="form-submit" method="post" action="<?= site_url("student/remove_from_learn"); ?>" >
												<input type="hidden" name="code" value="<?=$crs->course_code;?>" >
												<button type="submit" class="btn btn-colored btn-theme-colored2 text-white btn-lg btn-block" data-loading-text="Please Wait..!">REMOVE COURSE</button>
												</form>
											</td>
											<td>
												<a href="http://study.cftedu.in/" class="btn btn-colored btn-theme-colored2 text-white btn-lg btn-block"> Start </a>
											</td>
									</tr>
							<?php } } ?>
							</tbody>
					...
					<div class="col-md-12">
						...
						<table class="table table-striped table-bordered tbl-shopping-cart">
							...
							<tbody>
							<?php if(count($select_course)){ foreach($select_course as $crs){ ?>
									<tr>
										<td class="product-thumbnail"><img alt="<?=$crs->course_code;?>" title="<?=$crs->title;?>" src="<?=base_url("images/".$image_detail[$crs->img_id]);?>"></td>
										<td><?= strtoupper($crs->title); ?></td>
										<td>
											<form class="form-submit" method="post" action="<?= site_url("student/add_to_learn"); ?>" >
											<input type="hidden" name="code" value="<?=$crs->course_code;?>" >
											<button type="submit" class="btn btn-colored btn-theme-colored2 text-white btn-lg btn-block" data-loading-text="Please Wait..!">ADD TO LEARN</button>
											</form>
										</td>
									</tr>
							<?php } } ?>
